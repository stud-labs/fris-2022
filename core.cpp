...
double FRIS::fun(size_t x1, size_t x2, size_t u){ //функци€ конкурентного сходства
...
double FRIS::rFun(size_t x1, size_t u){ // функци€ конкурентного сходства по неклассифицированной выборке
      double r = 1000000;               // рассто€ние до виртуального образа
...
double FRIS::diss(size_t row1, size_t row2) { //вычисление дистанции между точками
    ...
    //устанавливаем статическое планирование распределени€ итераций по потокам (делим общее число итераций на потоки),
    //каждый поток будет выполн€ть примерно одинаковое количество итераций
    // #pragma omp parallel for //ordered schedule(dynamic) // замедл€ет программу
...
bool FRIS::calcdiss() {
    ...
    (*m_diss)(0, 0)=0;
    for (size_t row=1; row<h; row++) {               //по р€ду
        (*m_diss)(row, row)=0;
...
ssize_t FRIS::findNearest(size_t c, size_t u, bool inc, bool ext, bool unc) {
    // c>=0 ищет элемент с минимальной дистанцией до u в классе с (not=false, unc=false) »Ћ»
...
double FRIS::frisClus(ssize_t c1, ssize_t c2, size_t u) {
  ...
  // double r1 = min((*m_diss)(x1,u),(*m_diss)(x2,u)); // r1 дл€ алгоритма
...
//  ssize_t c1=-2;  // intermediate class
    ...
    for (size_t row=0; row<h; row++) {
        if (m_class[row]==c) {
           ...
           // cout<<"c["<<row<<"]="<<f<<endl; // выводить дл€ проверки
            s=s+f;
            t++;
        }
    }
